...
Options:
	...
	-c	:	Control fasta with wt sequences [required]
...
while getopts :q:c:d:o:p:t:k:h: opt; do
  case $opt in
	q)
	...
	;;
	c)
                echo "-c (control query) was input as $OPTARG" >&2
                CQUERY=$OPTARG
        ;;
	d)
...
if [ $QUERY == 0 ] ; then
	...
	-num_threads $THREADS | sort | uniq > ${OUTPUT}/${PREFIX}.out
	
	#control query blast
	blastn -db ${OUTPUT}/blastdb/${PREFIX} \
	-query $CQUERY \
        -outfmt "6 qseqid sseqid pident mismatch gapopen qstart qend sstart send evalue length qcovs qcovhsp sstrand" \
        -num_threads $THREADS | sort | uniq > ${OUTPUT}/${PREFIX}_c.out

	#remove previous gff3
	[ -e ${OUTPUT}/${PREFIX}.gff3 ] && rm ${OUPUT}/$PREFIX.gff3

	cat <(echo "##gff-version 3")\
		<(awk -v OFS='\t' '{if ($14 == "plus") print $2,"integration","mRNA",$8,$9,$10,"+","0","ID="NR";Name="$1";id="$3";cov="$12";covhsp="$13";len_align="$11;
		else print $2,"integration","mRNA",$9,$8,$10,"-","0","ID="NR";Name="$1";id="$3";cov="$12";covhsp="$13";len_align="$11;}' ${OUTPUT}/$PREFIX.out) > ${OUTPUT}/${PREFIX}.gff3

	cat <(echo "##gff-version 3")\
		<(awk -v OFS='\t' '{if ($14 == "plus") print $2,"control_integration","mRNA",$8,$9,$10,"+","0","ID="NR"cont;Name="$1";id="$3";cov="$12";covhsp="$13";len_align="$11;
		else print $2,"control_integration","mRNA",$9,$8,$10,"-","0","ID="NR"cont;Name="$1";id="$3";cov="$12";covhsp="$13";len_align="$11;;}' ${OUTPUT}/${PREFIX}_c.out) > ${OUTPUT}/${PREFIX}_c.gff3

	cat ${OUTPUT}/${PREFIX}.gff3 \
		<(bedtools subtract -a ${OUTPUT}/${PREFIX}_c.gff3 -b ${OUTPUT}/${PREFIX}.gff3) | sort | uniq > ${OUTPUT}/${PREFIX}2.gff3
